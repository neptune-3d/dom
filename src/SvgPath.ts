...
/**
 * Wrapper for the `<path>` SVG element, extending `BaseSvg<"path">` with path-specific functionality.
 *
 * Provides a fluent API for setting the `d` attribute using either raw path strings or structured `PathData` builders.
 * Inherits common styling and transform methods from `BaseSvg`, enabling consistent manipulation across SVG shape elements.
 *
